...
import me.superskidder.lune.utils.PlayerUtil;
...
public class PluginManager {
    ...
    public List<URLClassLoader> urlCL = new ArrayList<>();
    ...
    public void loadPlugins(boolean reload){
        try {
            ...
            for (File file : files) {
                urlCL.add(new URLClassLoader(new URL[]{new URL("file:" + file.getPath())}));
            }

            loading: for (int i = 0; i < urlCL.size(); i++) {
                URLClassLoader loader = urlCL.get(i);
                Class<?> clazz;
                try {
                    // 写死的Main 不能动
                    clazz = loader.loadClass("lune.PluginMain");
                    ...
                    node.methods.forEach(methodNode -> {
                        for (AbstractInsnNode insnNode : methodNode.instructions.toArray()) {
                            if (insnNode instanceof MethodInsnNode) {
                                ...
                                boolean flag = "me/qianxia/lune/manager/ModuleManager".equals(method.owner) && "addModule".equals(method.name) && "(Lme/qianxia/lune/Modules/Mod;)V".equals(method.desc);
                                if (flag) {
                                    method.name = "addPluginModule";
                                    method.desc = "(Lme/qianxia/lune/Modules/Mod;Ljava/lang/Object;)V";
